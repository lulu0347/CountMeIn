function post() {
    ...
    var questionCreator = $("#questionCreator").val();
    var questionTitle = $("#questionTitle").val();
    var loginMemNo = $("#loginMemNo").val();
    var loginMemName = $("#loginMemName").val();
    var content = $("#comment_content").val();
    var memNo = $("#memNo").val();
    comment2target(questionId, 1, content, memNo, questionCreator, questionTitle, loginMemNo, loginMemName);
}

function comment2target(questionId, type, content, memNo, questionCreator, questionTitle, loginMemNo, loginMemName) {
    if (!content) {
        alert("不能回覆空內容~~~");
        return;
    }
    ...
    $.ajax({
        ...
        data: {
            "notifier":loginMemNo,
            "notifierName": loginMemName,
            "receiver":questionCreator,
            "outerid": questionId,
            "outerTitle": questionTitle,
            "type": "1",
            "action": "insert"
        },
            ...
            $.getJSON("/comment/" + id, function (data) {
                $.each(data.data.reverse(), function (index, comment) {
                    var mediaLeftElement = $("<div/>", {
                        ...
                        "src": comment.userAccount.avatarUrl
                    }));
